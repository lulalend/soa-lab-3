...
public class BandSpecifications {
                ...
                for (String filter : filters) {
                    ...
                    if (fieldType.equals(Long.class)) {
                        try {
                            ...
                            throw new InvalidParameterException("Value for '" + field + "' is too large for type Long: " + value);
                        }
                    } else if (fieldType.equals(Integer.class)) {
                        try {
                            Integer.parseInt(value);
                        } catch (NumberFormatException e) {
                            throw new InvalidParameterException("Value for '" + field + "' is too large for type Integer: " + value);
                        }
                    } else if (fieldType.equals(Double.class)) {
                        try {
                            Double.parseDouble(value);
                        } catch (NumberFormatException e) {
                            throw new InvalidParameterException("Value for '" + field + "' is too large for type Double: " + value);
                        }
                    }
    ...
    private static Predicate handleEqual(Path<?> currentPath, Class<?> fieldType, String value, CriteriaBuilder criteriaBuilder) {
        if (fieldType.equals(LocalDate.class)) {
            ...
            try {
                ...
                throw new InvalidParameterException("Invalid enum value provided. Please check your input.");
            }
    ...
    private static Predicate handleGreaterThan(Path<?> currentPath, Class<?> fieldType, String value, CriteriaBuilder criteriaBuilder) {
        if (fieldType.equals(Long.class)) {
            ...
            return criteriaBuilder.greaterThan(currentPath.as(BigDecimal.class), new BigDecimal(value));
    ...
    private static Predicate handleGreaterThanOrEqualTo(Path<?> currentPath, Class<?> fieldType, String value, CriteriaBuilder criteriaBuilder) {
        if (fieldType.equals(Long.class)) {
            ...
            return criteriaBuilder.greaterThanOrEqualTo(currentPath.as(BigDecimal.class), new BigDecimal(value));
        } else if (fieldType.equals(LocalDate.class)) {
    ...
    private static Predicate handleLessThan(Path<?> currentPath, Class<?> fieldType, String value, CriteriaBuilder criteriaBuilder) {
        if (fieldType.equals(Long.class)) {
            ...
            return criteriaBuilder.lessThan(currentPath.as(BigDecimal.class), new BigDecimal(value));
        } else if (fieldType.equals(LocalDate.class)) {
    ...
    private static Predicate handleLessThanOrEqualTo(Path<?> currentPath, Class<?> fieldType, String value, CriteriaBuilder criteriaBuilder) {
        if (fieldType.equals(Long.class)) {
            ...
            return criteriaBuilder.lessThanOrEqualTo(currentPath.as(BigDecimal.class), new BigDecimal(value));
        } else if (fieldType.equals(LocalDate.class)) {
